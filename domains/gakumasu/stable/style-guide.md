...
完成 B/C 初稿后，**必须从头到尾重新通读一遍译文**，专门审查中文是否生硬，而不是只检查漏译和术语。自审时按以下 prompt 执行：

> 现在把译文当作中文成稿阅读。找出所有像是按日语语序、日语分句、日语搭配硬搬过来的地方；在不加戏、不删信息、不改变人物口吻和情绪强度的前提下，合并、拆分或重排相邻句子，让它读起来像自然的中文汉化文本。重点检查对话是否像角色会说出口的话，旁白是否有“的/了/着”堆叠、抽象名词硬译、比喻搭配不顺、以及 B/C 之间因字幕拆分造成的生硬表达。润色后同步更新 B 与 C，再重新运行转换脚本生成 D。
