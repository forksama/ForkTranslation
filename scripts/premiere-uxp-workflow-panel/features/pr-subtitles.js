...
(function registerPrSubtitles(app) {
  if (!app) {
    throw new Error("ForkTranslation workflow app shell is missing.");
  }

  app.registerFeature("subtitles", {
    init() {
      ...
      function $(elementId) {
        return app.$("subtitles", elementId);
      }
      ...
      function setBusy(busy) {
        state.busy = busy;
        setDisabled("chooseJsonButton", busy);
        setDisabled("chooseOutputButton", busy);
      ...
      async function generateRoleSrts(jsonFile, outputFolder, writeLog) {
        ...
        const cues = getOrderedCues(data);
        const markerInfo = await getSequenceMarkers(sequence);
        ...
        report(`Sequence markers: ${markerInfo.rawCount}`);
        if (markerInfo.duplicateCount > 0) {
          report(
            `Unique marker times: ${markers.length} ` +
              `(${markerInfo.duplicateCount} duplicate marker(s) ignored).`
          );
        }
        ...
        return {
          ...
          rawMarkerCount: markerInfo.rawCount,
          duplicateMarkerCount: markerInfo.duplicateCount,
          usableCueCount,
          writtenFiles,
          importedToProject
        };
      ...
      async function getSequenceMarkers(sequence) {
        ...
        rawMarkers.sort((a, b) => a.seconds - b.seconds);
        const markers = [];
        let duplicateCount = 0;

        for (const marker of rawMarkers) {
          const previous = markers.length > 0 ? markers[markers.length - 1] : null;
          if (
            previous &&
            Math.abs(marker.seconds - previous.seconds) <= MARKER_TIME_TOLERANCE_SECONDS
          ) {
            duplicateCount += 1;
            continue;
          }
      ...
      const chooseJsonButton = optional$("chooseJsonButton");
      if (chooseJsonButton) {
        chooseJsonButton.addEventListener("click", chooseJsonFile);
      }

      const chooseOutputButton = optional$("chooseOutputButton");
      if (chooseOutputButton) {
      ...
      app.setFeatureApi("subtitles", {
        async readCues() {
          ...
          return getOrderedCues(
            parseJson(await readTextFile(state.jsonFile), state.jsonFile.name)
          );
        },
        async generateSrts() {
          if (!state.jsonFile) {
