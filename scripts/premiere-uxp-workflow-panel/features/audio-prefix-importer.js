...
(function registerAudioPrefixImporter(app) {
  if (!app) {
    throw new Error("ForkTranslation workflow app shell is missing.");
  }

  app.registerFeature("audio", {
    init() {
      ...
      const MARKER_TIME_TOLERANCE_SECONDS = 0.002;
      ...
      function setBusy(busy) {
        state.busy = busy;
        setDisabled("chooseFolderButton", busy);
        setDisabled("rescanButton", busy);
      ...
      async function insertAudioAndMarkers(project, sequence, placements, target, options, addMarkers) {
        const shouldAddMarkers = addMarkers !== false;
        ...
        for (const strategy of strategies) {
          ...
          try {
              ...
              if (shouldAddMarkers) {
                try {
                  ...
                  const markerError = new Error(
                    "Audio clips were placed, but marker creation failed: " +
                      (error && error.message ? error.message : error)
                  );
                  markerError.stopPlacementFallback = true;
                  throw markerError;
                }
              }
              return {
            ...
            if (error && error.stopPlacementFallback) {
              throw error;
            }
            lastError = error;
      ...
      async function addPlacementMarkers(project, sequence, placements, options) {
        ...
        const markerTimes = await readSequenceMarkerTimes(sequence);
        const markerSpecs = [];
        let skippedMarkerCount = 0;

        for (const placement of placements.items) {
          if (hasMarkerAtTime(markerTimes, placement.startSeconds)) {
            skippedMarkerCount += 1;
            log(
              `Marker already exists at ${formatSeconds(placement.startSeconds)}; ` +
                `skipping ${markerNameForAudio(placement.file)}.`
            );
            continue;
          }

          markerTimes.push(placement.startSeconds);
          markerSpecs.push({
            name: markerNameForAudio(placement.file),
            time: placement.startTime,
            comment: `start: ${placement.file.name}`
          });
        }

        if (hasMarkerAtTime(markerTimes, placements.finalMarkerSeconds)) {
          skippedMarkerCount += 1;
          log(`Marker already exists at ${formatSeconds(placements.finalMarkerSeconds)}; skipping audio batch end.`);
        } else {
          markerTimes.push(placements.finalMarkerSeconds);
          markerSpecs.push({
            name: "audio batch end",
            time: placements.finalMarkerTime,
            comment: `last audio end + ${options.intervalSeconds}s`
          });
        }

        if (markerSpecs.length === 0) {
          log("No new markers were added because matching marker times already exist.");
          return;
        }
        ...
        log(`Added ${markerSpecs.length} new marker(s); skipped ${skippedMarkerCount} duplicate marker time(s).`);
      }

      async function readSequenceMarkerTimes(sequence) {
        ...
        for (const marker of markerItems) {
          ...
          if (start && Number.isFinite(Number(start.seconds))) {
            times.push(Number(start.seconds));
          }
        }
      ...
      function hasMarkerAtTime(markerTimes, seconds) {
        return markerTimes.some((time) => {
          return Math.abs(Number(time) - Number(seconds)) <= MARKER_TIME_TOLERANCE_SECONDS;
        });
      }
      ...
      const chooseFolderButton = optional$("chooseFolderButton");
      if (chooseFolderButton) {
        chooseFolderButton.addEventListener("click", chooseFolder);
      }

      const rescanButton = optional$("rescanButton");
      if (rescanButton) {
      ...
      async function importAudioByRole(plan, routing) {
        const options = readOptions();
        const files = await ensureAudioFiles();
        if (files.length !== plan.rows.length) {
          throw new Error(
            `Audio file count ${files.length} does not match plan row count ${plan.rows.length}.`
          );
        }
        ...
        log("");
        log("Script: ForkTranslation Audio Prefix Importer - Role Tracks");
        log(`Project: ${project.path || project.name || "(unsaved)"}`);
        log(`Sequence: ${safe(sequence.name)}`);
        ...
        const importPaths = files.map((file) => file.path);
        const rootItem = await project.getRootItem();
        const imported = await project.importFiles(importPaths, true, rootItem, false);
        if (!imported) {
          throw new Error("Project.importFiles returned false.");
        ...
        const projectItems = await resolveProjectItemsForFiles(project, files);
        const placements = await buildPlacements(projectItems, startTime, options.intervalSeconds);
        const timelinePlan = attachPlacementTimes(plan, placements);
        const groups = groupPlacementsByRole(placements.items, timelinePlan.rows);

        for (const group of groups) {
          const route = routing[group.role];
          if (!route || !route.audioTrack) {
            throw new Error(`No audio track was configured for role: ${group.role}`);
          }

          const target = await resolveTargetAudioTrack(sequence, route.audioTrack);
          log(`Role ${group.role}: ${group.items.length} audio clip(s) -> ${target.label}`);
          await insertAudioAndMarkers(
            project,
            sequence,
            {
              items: group.items,
              finalMarkerSeconds: placements.finalMarkerSeconds,
              finalMarkerTime: placements.finalMarkerTime
            },
            target,
            options,
            false
        ...
        const text =
          `Done. Placed ${placements.items.length} role-routed audio file(s); ` +
          `created ${placements.items.length + 1} marker boundary time(s).`;
        setStatus(text);
        log(text);
      ...
      function attachPlacementTimes(plan, placements) {
        const rows = plan.rows.map((row, index) => {
          const placement = placements.items[index];
          const nextPlacement = index + 1 < placements.items.length
            ? placements.items[index + 1]
            : null;
          const targetEndSeconds = nextPlacement
            ? nextPlacement.startSeconds
          ...
          return {
            ...
            endTime: ppro.TickTime.createWithSeconds(placement.endSeconds),
            targetEndTime: ppro.TickTime.createWithSeconds(targetEndSeconds)
          };
        });
      ...
      function groupPlacementsByRole(placements, rows) {
        ...
        for (let index = 0; index < placements.length; index += 1) {
          ...
          group.items.push(placements[index]);
        }
